...
Features:
 ...
 17. Self Security (runs a security scan simulation using process data).
 18. Anti Graffer (checks for known screenshot/grabber processes).
 19. Anti Graffer v2 Beta (advanced check with CPU usage of suspicious processes).
...
def print_boxed(text, width=BOX_WIDTH, padding=1, border_color=Fore.RED):
    ...
    :param text: Text to print (can include newlines).
    :param width: Overall width of the box.
    :param padding: Empty lines above and below the text.
...
def convert_py_to_exe():
    ...
    if process.returncode == 0:
        ...
        if exe_file.exists():
            ...
            print(Fore.RED + "\nExecutable file not found in the output directory. Check logs for errors.")
    else:
    ...
    print("\n🎉 Process completed! Please check the output directory for your executable.")
...
def bluetooth_spam():
    """Sends Bluetooth messages to a selected device using RFCOMM (if supported)."""
    ...
    try:
        ...
        sock.connect((target[0], 1))  # Using RFCOMM channel 1
...
def self_security():
    """Performs a self security check by scanning running processes for common vulnerabilities."""
    print_boxed("Self Security Check", width=BOX_WIDTH)
    print("Performing security scan...\n")
    # Example: Check for processes that might indicate a vulnerable system (this is just an example)
...
def anti_graffer_v2_beta():
    """Performs an advanced scan for suspicious screen grabber processes (Beta)."""
...
def about_xe_tools():
    ...
    about_text = (
        ...
        "- Perform a self security check by scanning for vulnerable processes.\n"
        "- Check for suspicious screen capture (graffer) tools (basic and advanced beta).\n\n"
        "Community:\n"
